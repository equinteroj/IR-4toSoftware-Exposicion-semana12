...
\begin{titlepage}
 \centering
   ...
   \textbf{Link de GitHub}\\[0.5cm]

\url{https://github.com/equinteroj/IR-4toSoftware-Exposicion-semana12.git}\\[0.5cm]
...
Se espera que el equipo documente aquí, tras ejecutar la generación real, qué elementos del modelo \emph{no} fueron generados automáticamente y requirieron completarse a mano (por ejemplo: lógica de validación de negocio compleja como la comparación contra rangos óptimos de RF-10, configuración del \texttt{persistence.xml}, o la conexión efectiva a PostgreSQL/MySQL). El código escrito manualmente debe distinguirse claramente del código generado mediante carpetas separadas, comentarios o marcadores de bloque protegido.
...
\placeholder{captura del diagrama de clases ANTES del cambio (clase Grameo sin el atributo observaciones).}

\placeholder{captura del diagrama de clases DESPUÉS del cambio (clase Grameo con el atributo observaciones agregado).}

\placeholder{captura del código Java de la clase Grameo ANTES de la regeneración.}

\placeholder{captura del código Java de la clase Grameo DESPUÉS de la regeneración, mostrando el nuevo campo (y, si aplica, el bloque protegido con código manual conservado).}
